...
def notices(request):
    ...
    unseen = Notice.objects.notices_for(request.user, on_site=True, unseen=True)
    seen = Notice.objects.notices_for(request.user, on_site=True, unseen=False)
...
def mark_seen(request):
    """
    Mark given unseen notice in GET['id'] as seen. Returns id of changed notice
    if succeed else return empty string.
    """
